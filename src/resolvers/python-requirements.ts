...
import type { Package, Resolution, Resolver } from "../types/index.js";

export class PythonRequirementsResolver implements Resolver {
  canResolve(filename: string): boolean {
    const basename = filename.split("/").pop() || filename;
    return /^requirements.*\.(txt|lock)$/i.test(basename);
  }
